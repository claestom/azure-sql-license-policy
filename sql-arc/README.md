...
This solution deploys and remediates a custom Azure Policy that configures and enforces the Arc-enabled SQL Server extension `LicenseType` to a selected target value (for example `Paid` or `PAYG`).
